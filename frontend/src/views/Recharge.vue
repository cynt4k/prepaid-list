  ...
  <v-container class="home" fluid fill-height>
    ...
    >
      <div class="numpad">
        <div class="output">
          ...
          <v-btn text icon @click="removeValue()">
    ...
    <v-dialog v-model="confirmationDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-actions>
          <v-btn color="red" text @click="cancel()">Abbrechen</v-btn>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="accept()">Bestätigen</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <confirmation-dialog
...
export default class Recharge extends Vue {
  ...
  private accept() {
    ...
    Factory.getInstance()
      .ProfileService.addBalance(balance)
      .subscribe(
        (infos: IUserModel) => {
          userStore.updateBalance(infos.balance);
          this.confirmationDialog = false;

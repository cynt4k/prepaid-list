  ...
  <v-container class="home" fluid fill-height>
    ...
    <v-dialog v-model="dialogExtraProduct" v-if="selectedProduct">
      <v-card>
        ...
        <v-card-actions>
          <v-btn color="primary" text @click="dialogExtraProduct=false">Schließen</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default class SingleProducts extends Vue {
    ...
    private mounted() {
      ...
      Factory.getInstance().ProductService.getProductsByCategory(this.category).subscribe(
